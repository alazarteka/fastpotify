...
pub fn show(app: &mut App, ui: &mut egui::Ui) {
    ...
    section(ui, &palette, "Account", |ui| {
        ...
        widgets::setting_row(
            ...
            &palette,
            "Make it even faster",
            "Spotify limits each app, and everyone shares this one. Add an app of your own for eligible requests; the shared app keeps full playlist coverage. Paste its Client ID here.",
            |ui| {
        ...
        // Authorization is explicit: editing the field never opens a browser.
        let wanted = app.settings.web_client_id.as_deref();
        let in_use = wanted.is_some() && app.web_app.as_deref() == wanted;
        if in_use {
            widgets::setting_row(
                ui,
                &palette,
                "Your app is authorized",
                "Eligible requests use its independent limit; the shared app stays connected.",
                |ui| {
            ...
            widgets::setting_row(
                ...
                &palette,
                "Authorize your app",
                "Spotify opens once to add this independent Web API session.",
                |ui| {
            ...
            widgets::setting_row(
                ui,
                &palette,
                "Remove personal authorization",
                "The shared Web API session remains connected.",
                |ui| {
                    if theme::pill_button(ui, &palette, "Remove", false).clicked() {
                        app.actions.push(Action::ConfigurePersonalWebApp);
                    }
            ...
            |ui| {
                ui.horizontal(|ui| {
                    ...
                    if theme::soft_button(ui, &palette, None, "-", false).clicked() {
                        zoom = (zoom - 0.1).max(crate::settings::ZOOM_MIN);
                    }
                    theme::text(
                        ui,
                        format!("{:.0}%", zoom * 100.0),
                    ...
                    if theme::soft_button(ui, &palette, None, "+", false).clicked() {
                        zoom = (zoom + 0.1).min(crate::settings::ZOOM_MAX);
                    }
